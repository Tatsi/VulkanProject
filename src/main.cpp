...
private:
    ...
    void createInstance()
    {
        ...
        // Get extensions that are required by our app
        std::vector<const char*> requiredExtensions = getRequiredExtensions();

        // Check that all extensions required by our apps are supported
        for (const char* extension : requiredExtensions)
        {
            const std::string extensionName = std::string(extension);
            if (std::find_if(requiredExtensions.begin(), requiredExtensions.end(), [extensionName](const char* p)
                             { return std::string(p) == extensionName; }) == requiredExtensions.end())
            {
                throw std::runtime_error("Required extension " + extensionName + " is not supported!");
            }
        }
